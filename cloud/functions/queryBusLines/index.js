...
exports.main = async (event, context) => {
  ...
  const docList = await db
    ...
    .where({
      cycle:
        startDay === "1,2,3,4,5"
          ? db.command.or(
              db.command.eq("1,2,3,4,5"),
              db.command.eq("1,2,3,4,5,6,7"),
            )
          : db.command.or(
              db.command.eq("6,7"),
              db.command.eq("1,2,3,4,5,6,7"),
  ...
  const res = docList.filter((item) => {
    ...
    for (let i = 0; i < sz; ++i) {
      ...
      if (
        station["name"].indexOf(startAddress) !== -1 &&
      ...
      if (
        station["name"].indexOf(endAddress) !== -1 &&
        (!station["time"] || station["time"] >= startTime)
      ) {
        endOnlyFit = true;
      }
    ...
    if (startFit && !endFit && item.endStation.indexOf(startAddress) === -1) {
      ...
      !startFit &&
      endOnlyFit &&
      item.startStation.indexOf(endAddress) === -1
    ) {
      endList.push(item);
    }
  ...
  if (canTrans) {
    ...
    for (let i = 0; i < m; ++i) {
      ...
      if (matchedRes.length > 0) {
        const sorted = matchedRes.sort((item, jtem) =>
          item.startTime < jtem.startTime ? -1 : 1,
        );
